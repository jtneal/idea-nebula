...
@Component({
  ...
  template: `
    <div 
      class="idea-card"
      [style.border-left]="category ? '4px solid ' + category.color : '4px solid rgba(255,255,255,0.3)'"
      [style.background]="getCardBackground()"
    >
      <div class="card-header">
        <div class="category-badge" *ngIf="category" [style.background-color]="category.color">
          {{ category.title }}
        </div>
        <button class="delete-btn" (click)="onDelete()" title="Delete idea">×</button>
      </div>

      <div *ngIf="!isEditing" class="view-mode" (click)="startEditing()">
        <h3 class="idea-title">{{ idea.title || 'Untitled Idea' }}</h3>
        <div class="idea-description" [innerHTML]="formatDescription(idea.description)"></div>
        <div class="idea-meta">
  ...
  styles: [`
    ...
    .title-input, .description-input, .category-select {
      width: 100%;
      padding: 12px;
    ...
    .title-input:focus, .description-input:focus, .category-select:focus {
      outline: none;
...
export class IdeaCardComponent implements OnInit {
  ...
  formatDescription(description: string): string {
    if (!description) {
      return '<span style="color: rgba(255,255,255,0.5); font-style: italic;">Click to add description...</span>';
    }
